...
// sometimes the unit tests fail, i don't know why, but it works 100% on my local machine
...
let Translator;
let translate;
// regex for cleaning the output so the tests will pass
  ...
  suite('Function translate()', () => {
    suite('American to British English', () => {
      ...
      test('Mangoes are my favorite fruit. --> Mangoes are my favourite fruit.', done => {
        ...
        // I always need to clean the output, since the string is modified so it's correctly displayed in the div
        str = translate(input, "american-to-british").replace(regex, "");
    ...
    suite('British to American English', () => {
      ...
      test('We watched the footie match for a while. --> We watched the soccer match for a while.', done => {
        ...
        str = translate(input, "british-to-american").replace(regex, "");
        assert.equal(str, output)
        done();
      });
